...
%     [q0,q1,q2,q3]=Quaternion_FromEulerAngle(0,0,90*deg2rad);%初始四元数计算
    ...
    %     AHRS.UpdateIMU(Gyroscope(t,:), Accelerometer(t,:));	% gyroscope units must be radians
        ...
        if euler(t,3)<0
            euler(t,3)=euler(t,3)+360;
        end
%         if abs(euler(t:3)-euler(t-1,3))>100
%             t=t-1;
%             Pitch0=asin(ax(t)/sqrt(ax(t)*ax(t)+ay(t)*ay(t)+az(t)*az(t)));%初始角计算
%             Roll0=atan2(-ay(t),-az(t));
%             Yaw0=atan2(-my(t)*cos(Roll0)+mz(t)*sin(Roll0),mx(t)*cos(Pitch0)+my(t)*sin(Pitch0)*sin(Roll0)+mz(t)*sin(Pitch0)*cos(Roll0))-8.3*pi/180;
%             quaternion(t, :)=Quaternion_FromEulerAngle(Roll0,Pitch0,Yaw0);
%             euler(t,:)=quatern2euler(quaternConj(quaternion(t,:))) * (180/pi);
%         end
    end
...
%         YawCos = mean(yaw(pos_start:pos_end,3));
        YawSin = mean(yaw(pos_start,2));
        YawCos = mean(yaw(pos_start,3));
