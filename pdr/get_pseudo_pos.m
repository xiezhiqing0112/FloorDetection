function get_pseudo_pos(posi_p,file_p,save_p,file_n,x);
...
%     distance = 0;
    step_idx=[];
     ptime=[];
    pseudo_pos=zeros(floor(length(trial(:,2))/100), 2);
    ...
    for k = 1:StepCount-1
        ...
        if pos_end-pos_start>1000
            ...
            for i= round((pos_start)/100)*100:100: round((pos_end)/100)*100-100
                ...
                pstep_idx(pseudo_idx,1:2)=[i i+100];
                ptime(pseudo_idx)=trial(i,1);
                pseudo_pos(pseudo_idx+1,2) = pseudo_pos(pseudo_idx,2) + StepLength * sin(YawSin*deg2rad);
                pseudo_pos(pseudo_idx+1,1) = pseudo_pos(pseudo_idx,1) - StepLength * cos(YawCos*deg2rad);
                pseudo_idx=pseudo_idx+1;
            end
        end
    end
...
%         scatter(PositionX(st:ed),PositionY(st:ed),'k');
